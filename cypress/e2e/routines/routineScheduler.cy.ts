...
describe("Routine Scheduler Feature", () => {
  ...
  it("user can create and delete a schedule", () => {
    ...
    cy.createRoutine(
      ...
      cy.get("[data-cy=schedule-routine-length-input]").type(
        `${testSchedule.routineLengthMinutes}`,
      );
  ...
  it("user cannot create a schedule with no name or routine length", () => {
    ...
    cy.createRoutine(
      ...
      cy.intercept("POST", "**", (req) => {
        throw new Error(
          `POST request to ${req.url} should not have been called`,
        );
      }).as("anyPostRequest");
  ...
  it("user can edit a schedule", () => {
    ...
    cy.createSchedule(
      testUser.id,
      testSchedule.name,
      testSchedule.routineIds,
      testSchedule.routineLengthMinutes,
      testSchedule.dayOfWeek,
      testSchedule.timeSlot,
    ).then((scheduleId) => {
      const editedSchedule = {
        name: "edited name here",
      ...
      cy.visit("/routine/scheduler");
      cy.contains("tr", testSchedule.name).click();
      cy.get("[data-cy=edit-schedule-name-input]")
        .should("have.value", testSchedule.name)
        .clear()
        .type(editedSchedule.name);
      cy.get("[data-cy=edit-schedule-routine-length-input]")
        .should("have.value", testSchedule.routineLengthMinutes)
        .clear()
        .type(`${editedSchedule.routineLengthMinutes}`);
      cy.get("[data-cy=edit-schedule-day-select]")
        .should("have.value", testSchedule.dayOfWeek)
        .select(editedSchedule.dayOfWeek);
      cy.get("[data-cy=edit-schedule-time-slot-input]")
        .should("have.value", testSchedule.timeSlot)
        .clear()
        .type(editedSchedule.timeSlot);
      cy.get("[data-cy=edit-schedule-btn]").click();
      cy.wait("@editScheduleRequest")
        .its("response.statusCode")
        .should("eq", 200);

      // now delete the schedule after checking the fields were edited to the new values
      cy.contains("tr", editedSchedule.name).click();
      cy.get("[data-cy=edit-schedule-name-input]").should(
        "have.value",
        editedSchedule.name,
      );
      cy.get("[data-cy=edit-schedule-routine-length-input]").should(
        "have.value",
        editedSchedule.routineLengthMinutes,
      );
      cy.get("[data-cy=edit-schedule-day-select]").should(
        "have.value",
        editedSchedule.dayOfWeek,
      );
      cy.get("[data-cy=edit-schedule-time-slot-input]").should(
        "have.value",
        editedSchedule.timeSlot,
      );
      cy.get("[data-cy=delete-schedule-btn]").click();
      cy.wait("@deleteScheduleRequest")
        .its("response.statusCode")
