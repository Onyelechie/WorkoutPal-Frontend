...
Cypress.Commands.add("deleteRoutine", (routineId) => {
  cy.request("DELETE", `${BACKEND_URL}/routines/${routineId}`).then(
    (response) => {
...
Cypress.Commands.add(
  "createSchedule",
  (userId, name, routineIds, routineLengthMinutes, dayOfWeek, timeSlot) => {
    return cy
      .request("POST", `${BACKEND_URL}/schedules`, {
        userId: userId,
        name: name,
...
Cypress.Commands.add("deleteSchedule", (scheduleId) => {
  cy.request("DELETE", `${BACKEND_URL}/schedules/${scheduleId}`).then(
    (response) => {
      expect(response.status).equal(200);
    },
  );
...
Cypress.Commands.add("createUser", (user) => {
  return cy
    .request("POST", `${BACKEND_URL}/users`, {
      name: user.name,
...
Cypress.Commands.add("deleteUser", (userId, email, password) => {
  cy.request("POST", `${BACKEND_URL}/login`, { email, password }).then(() => {
    cy.request("DELETE", `${BACKEND_URL}/users/${userId}`).then((response) => {
      expect(response.status).equal(200);
    });
  });
});
