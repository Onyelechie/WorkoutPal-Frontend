...
export function AppComponent() {
    ...
    async function handleAction() {
        const result = await confirmDialog.showConfirm(titleString, messageString, positiveBtnString, negativeBtnString);
        if (result === true) {
...
```tsx
const result = await confirmDialog.showConfirmSafe(titleString, messageString, positiveBtnString, negativeBtnString);
```
![Confirm Dialog Safe Image](./ConfirmDialog/img-docs/ConfirmDialogSafe.png)

## Confirm Dialog Risky Action
```tsx
const result = await confirmDialog.showConfirmRisky(titleString, messageString, positiveBtnString, negativeBtnString);
```
