...
export default function EditScheduleModal({
  ...
  // state variable
  const [schedulePayload, setSchedulePayload] = useState<Schedule>({
    ...selectedSchedule,
  });
  ...
  async function handleEditSchedule(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await putRequest(
        `${ENDPOINT}/${schedulePayload.id}`,
        schedulePayload,
      );
      setSchedules((prev) =>
        prev.map(
          (s) => (s.id === response.data.id ? response.data : s), // replace the matching schedule
        ),
      );
      onClose();
    } catch (error: any) {
