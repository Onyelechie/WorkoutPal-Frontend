...
import { getLockedAchievements, unlockAchievement } from "../achievementService";
    ...
    describe("getLockedAchievements", () => {
        ...
        it("returns only achievements that are not unlocked", async () => {
            const unlocked: UserAchievementUnlocked[] = [
                { id: 1, title: "Achievement 1", description: "desc", badgeIcon: "1", date: "2025-01-01" },
            ];

            (getRequest as any).mockImplementation((endpoint: string) => {
                if (endpoint === "/achievements") return Promise.resolve({ data: allAchievements });
                if (endpoint === "/achievements/unlocked") return Promise.resolve({ data: unlocked });
    ...
    describe("unlockAchievement", () => {
        it("calls postRequest if the achievement is not unlocked", async () => {
            ...
            (postRequest as any).mockResolvedValue({
                data: { id: 2, title: "Achievement 2", description: "desc", badgeIcon: "2", date: "2025-11-04" },
            });

            const result = await unlockAchievement(2, 123);

            expect(postRequest).toHaveBeenCalledWith("/achievements", { achievementId: 2, userId: 123 });
            expect(result).toEqual({ id: 2, title: "Achievement 2", description: "desc", badgeIcon: "2", date: "2025-11-04" });
        });
